...
let newRoles = async(rol1,rol2) => { //FORMA DE CREAR UN NUEVO AUTHOR
    for (let author of rol1) {
        let user = await User.findOne({ email: author.user_id })
        author.user_id = user._id
        await Author.create(author)
    }
    for (let company of rol2) { // FORMA DE CREAR UNA NUEVA COMPAÑIA
...
let newMangas = async(mangas) => { //ESTA FUNCION ES PARA CREAR NUEVOS MANGAS, antes se tiene que fijar si existen, las categorias etc, lo van a ver mas adelante
